...
| [`gymrun-patch-band-recut-and-level-curve.md`](gymrun-patch-band-recut-and-level-curve.md) | patch prompt | `active` | the `rewardPools.ts` "a gym offer is strictly better than elite" rule, and `40dbb0b`'s `{1: 4, 2: 1}` opening weights | committed 2026-09-17 before any work, on `claude/admiring-euler-dhn536`. **A data-table patch, so nothing overlapping may be `active` beside it.** Three turns of prompts filed verbatim, plus the two planning clarifications and the provenance instruction that produced the report. Report, filed before any code and treated as a hard stop: [`../reports/early-game-band-and-curve.md`](../reports/early-game-band-and-curve.md) — it is also the decision record, and carries the first external reference the move ramp has ever had (gen-9 level-up learnsets for all 900 pool species). Moves `RANDOMIZER_VERSION` to `-19` and `contentHash`; `RUN_LOG_VERSION` and `AI_VERSION` hold. Deviations and the deleted invariant in `../generation.md` section 36 |
