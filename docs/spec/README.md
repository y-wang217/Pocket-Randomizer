...
| [`gymrun-patch-density-modes.md`](gymrun-patch-density-modes.md) | patch prompt | `draft` | | committed 2026-09-11 before any work, on `claude/bold-clarke-xcwko1`. Replaces the two valued 4.7.2 verbosity flag with a three valued density setting. Presentation only: no `core/` change, no version axis moves. Report on its four questions is due before any code |
