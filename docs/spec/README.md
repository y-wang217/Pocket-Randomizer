...
| [`gymrun-patch-ci-workflow.md`](gymrun-patch-ci-workflow.md) | patch prompt | `draft` | | committed 2026-09-17 before any work, on `claude/brave-hopper-th7one`. Build infrastructure only: `scripts/`, `package.json`, `.github/` and `docs/` — no `src/` change, no version axis moves, and `docs/visual/baseline/` untouched by the brief's own instruction. Splits the `npm run check` `&&` chain into a runner that reports every leg rather than the first failing one, then adds the GitHub Actions workflow. **One referent in the brief does not resolve** — part 2 says "per the structure above" and no structure was in the message — so it is recorded in the prompt file rather than guessed at, and part 2 waits on the author's answer |
