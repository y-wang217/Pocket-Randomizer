...
| [`gymrun-patch-ci-workflow.md`](gymrun-patch-ci-workflow.md) | patch prompt | `active` | | committed 2026-09-17 before any work, on `claude/brave-hopper-th7one`. **Part 1 built**, part 2 not started. Build infrastructure only: `scripts/`, `package.json`, `.github/` and `docs/` — no `src/` change, no version axis moves, and `docs/visual/baseline/` untouched by the brief's own instruction. Splits the `npm run check` `&&` chain into a runner that reports every leg rather than the first failing one, then adds the GitHub Actions workflow. Part 1's nine legs and the two kinds of SKIPPED are recorded in `../generation.md` section 30. **One referent in the brief does not resolve** — part 2 says "per the structure above" and no structure was in the message — so it is recorded in the prompt file rather than guessed at. The author supplied a YAML skeleton when asked, filed verbatim under the brief, and it names four scripts this tree did not have and one runner (`playwright test --project=`) that this repo does not use at all — part 1 lands three of the four names, and the runner mismatch is part 2's to reconcile |
