...
| [`gymrun-security-memo.md`](gymrun-security-memo.md) | memo prompt | `active` | | committed 2026-09-16 before any work, on `claude/determined-davinci-2bm0ch`, with the two questions asked and answered before any file was written. Produces [`../security.md`](../security.md), the `CLAUDE.md` section "Secrets and private material", `.env.example`, the `.gitignore` env lines and `test/security.test.ts`. Documents and one test: no `src/` change, no version axis moves. Adds protocol 8 below |
| [`gymrun-patch-daily-stats.md`](gymrun-patch-daily-stats.md) | patch prompt, scoped | `draft` | | committed 2026-09-16 before any work, on `claude/determined-davinci-2bm0ch`, with the four questions asked and answered and the scope report the owner asked to have written into the repo. **Scoped, not started**: nine half-days over six phases, the first of which is the security memo above and is built. Governed by [`../security.md`](../security.md): the migrations and RLS are public, the project's keys and thresholds are not, and `supabase/` is the first server setup this repository will carry. When built it moves no version axis and makes the project's first network call |
...
8. **A prompt that arrives carrying a secret is the one exception to
   "verbatim".** The secret is replaced in the filed copy by
   `[REDACTED: what it was]` and the register row says a redaction was made.
   Everything else in the prompt is untouched. [`../security.md`](../security.md)
   section 6.
