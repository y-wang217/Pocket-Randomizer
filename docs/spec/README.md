...
| [`gymrun-patch-density-modes.md`](gymrun-patch-density-modes.md) | patch prompt | `active` | | committed 2026-09-11 before any work, on `claude/bold-clarke-xcwko1`. Replaces the two valued 4.7.2 verbosity flag with a three valued density setting. Presentation only: no `core/` change, no version axis moves. Report [`../visual/reports/patch-density-modes.md`](../visual/reports/patch-density-modes.md); the six rulings on it are appended to the prompt file verbatim and their corrections recorded in `../generation.md` section 12l |
