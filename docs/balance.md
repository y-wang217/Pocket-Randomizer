...
| `randomizer-16` · `2eea4c`, 400 | `ai-6-spent-item`, pinned | RETUNE | **0.0%** | **0.46** | **Stage 4.9, first pass**, 2026-09-15, after the stage's three in-flight corrections (`generation.md` section 21). Gym clear rates of the runs that reached each: 42.5 / 49.0 / 20.5 / 44.4 / 50.0 / 100 / – / –; 285 of 400 reached gym 1 and 69.8% of all deaths are there. Mean turns per battle, segments 1-2: 3.70 / 7.06. 470 distinct species across 2866 encounters, 193 distinct starters; the most common species in 15.5% of runs against 31.3% at the baseline. **Recorded, not chased**: the stage asked for a measly start, a fierce two-Pokemon gym 1 on the hard AI, and opponents on the player's own evolution clock, and this is what that costs the greedy bot. The levers not pulled are the gym level column and the gym roster rule. Report `sim-reports/benchmarks/2026-09-15T17-02-59-607Z-gymrun-randomizer-16-400.json` |
| `randomizer-16` · `c3964b`, 400 | `ai-6-spent-item`, pinned | RETUNE | 0.0% | **0.46** | **the victory-order patch's own baseline**, 2026-09-17, measured on the patch's immediate parent (`a8db385`) before any of its code. Recorded because the nearest row above it is at `2eea4c`, a different hash again — reading this patch against that one would be reading across a yardstick that moved in between. Gym clear rates of the runs that reached each: 42.5 / 49.0 / 20.5 / 44.4 / 50.0 / 100 / – / – |
| `randomizer-17` · `73c1ee`, 400 | `ai-6-spent-item`, pinned | RETUNE | 0.0% | **0.47** | **the victory-order patch**, 2026-09-17. Read against the row directly above: **+0.01 mean gyms, completion unmoved.** Gym clear rates: 43.2 / 49.0 / 20.5 / 44.4 / 50.0 / 0 / – / – — the gym 6 column is one run either way and is noise, not a signal. **The battle pair is not in this number at all**: no seed in this population reaches segment 7, which is the only segment it touches, so the patch's one deliberate difficulty change is unmeasured here and will first appear in a population that gets that far. The reorder and the gym-move decline are both in it and moved nothing: the scripted baseline never declines, and the reorder changes which party a target index is resolved against rather than what the bot answers. Report `sim-reports/benchmarks/2026-09-17T04-01-54-236Z-gymrun-randomizer-17-400.json` |
...
| `randomizer-20` · `b8b419`, 400 | `ai-6-spent-item`, pinned | RETUNE | 0.0% | **0.655** | **the baseline for the teach-now and gym-level-spread patch**, 2026-09-18, measured on `claude/great-curie-99l9fm` at `1bdd8b1` — the protocol commit, before any code. Recorded because **the head of the tree had no row at all**: the R19 rulings took `RANDOMIZER_VERSION` to `-20` and `contentHash` to `b8b419` across three bumps and none of them was benchmarked, so the nearest row above is a different yardstick and reading the next patch against it would be reading across a stamp that moved in between. Read against that row anyway, for what it is worth: **+0.005 mean gyms, and the one completion in it is gone.** Gym clear rates of the runs that reached each: 58.5 / 59.8 / 56.5 / 61.1 / 77.8 / 42.9 / 0.0 / – on parties of 248 / 117 / 46 / 18 / 9 / 7 / 3 / 0, against 58.0 / 56.4 / 55.8 / 45.5 / 50.0 / 50.0 / 100 / 50.0 on 250 / 117 / 52 / 22 / 8 / 4 / 2 / 2. **Gyms 1 and 2 are flat** — +0.5pt and +3.4pt on the only two columns with more than fifty parties behind them — which is what the R19 rulings' own accounts predict: gym held items and the technique price are late and shop-side, and this population barely reaches either. Everything from gym 4 rightward is three to nine seeds and the 100% and 50% figures in the row above were one and two runs. **Recorded, not chased**: nothing was tuned for this run and no change of this patch is in it. Report `sim-reports/benchmarks/2026-09-18T17-10-55-820Z-gymrun-randomizer-20-400.json` |
| `run-20` · `b8b419`, 400 | `ai-6-spent-item`, pinned | RETUNE | 0.0% | **0.660** | **teach now or store**, 2026-09-18. The first row in this table stamped by `RUN_LOG_VERSION` rather than `RANDOMIZER_VERSION`, because that is the only axis this patch moved — `randomizer-20` and `b8b419` are unchanged from the row above, which is what makes the two comparable at all. Read against it: **+0.005 mean gyms, completion unmoved at zero.** Gym clear rates 58.0 / 58.1 / 53.8 / 59.1 / 63.6 / 42.9 / 0.0 / – on parties of 250 / 117 / 52 / 22 / 11 / 7 / 3 / 0, against 58.5 / 59.8 / 56.5 / 61.1 / 77.8 / 42.9 / 0.0 / – on 248 / 117 / 46 / 18 / 9 / 7 / 3 / 0. **It is flat, and that is the finding.** Every column with more than fifty parties behind it moves by two points or less in the *negative* direction while the mean moves up by five thousandths; gym 5's -14.2pt is nine parties becoming eleven. Nothing here is a signal. **The mechanism is not in doubt** — it is measured directly rather than through this number: of runs that ever hold a TM, the share that ever reach a boundary where one can be spent went **30.6% to 100%**, and it cannot be otherwise, because a node that pays a move now allows that move to be taught there. The sim fixture and the `SEED-A` baseline both show a teach landing that previously did not, and `SEED-A` clears a gym it used to die at. **So the honest reading is that the greedy baseline gains nothing from spending a TM earlier**, and two reasons are available without being distinguishable at this sample: 69.8% of deaths are at gym 1 and most TMs arrive after it, and `greedyMoveToReplace` displaces the weakest damaging move whether or not the incoming one is better. The second is a real candidate defect and is filed rather than chased. **Recorded, not chased**: no number was tuned against this run, and the patch was aimed at a capability the player did not have rather than at this figure. Report `sim-reports/benchmarks/2026-09-18T18-20-24-748Z-gymrun-randomizer-20-400.json` |
| `randomizer-21` · `d4e080`, 400 | `ai-6-spent-item`, pinned | RETUNE | **1.0%** | **1.085** | **the gym level spread**, 2026-09-18. Read against the row directly above, which is the same tree one commit earlier: **+0.425 mean gyms, a 64% rise, and completion off zero for only the second time since Stage 4.9.** Gym clear rates 73.6 / 75.7 / 67.9 / 80.4 / 63.6 / 73.7 / 57.1 / 66.7 against 58.0 / 58.1 / 53.8 / 59.1 / 63.6 / 42.9 / 0.0 / –, on parties of 250 / 144 / 84 / 46 / 33 / 19 / 14 / 6 against 250 / 117 / 52 / 22 / 11 / 7 / 3 / 0. **This is the largest single move in the table since Stage 4.8** and it is not carried by one column: gym 1 is +15.6pt on 250 parties, gym 2 +17.6pt on 144 and gym 3 +14.1pt on 84, which are three columns deep enough to mean something, and they agree. The party counts are the clearer reading — **every gym from the second on is reached by more runs than before**, 117 to 144 at gym 2 and 52 to 84 at gym 3, and gym 8 goes from a column no seed reached to six parties clearing four. **Four runs cleared all eight.** **The change is a shape rather than a discount, and that distinction is the whole patch.** The brief asked for a level off the gym mons; the reference says the gym-to-cap ratio is flat at 0.91 across all sixteen gyms of FireRed and Emerald, so a flat shave copies nothing. What was wrong was that every gym Pokemon sat at the player's cap when a real gym puts only its ace there. `levelOffset.gym.max` is still zero — no gym member is ever above the party — and what moved is `min`. Measured over 300 teams a segment the mean lands 0.895 to 0.925 against the reference's 0.91, and segment 0 fields `15,13` where Emerald's Roxanne fields 12 and 15. **A second effect rides with it and is not separable at this sample**: `rollSpec` clamps a drawn level up to its species' own evolution level, so the gym's roster is unchanged where a table-only edit would have cut Fire's top band from 21 species to 3 and emptied segment 6's band 4 entirely. Both landed in one commit and this row cannot say how the delta divides between them. **Recorded, not chased.** No number was tuned against this run; the column is `round(-0.18 x playerLevel)` derived from the reference mean and not fitted to any figure here. The standing ruling still applies — an earlier build averaged 3 gyms and cleared all 8 consistently, which is the failure mode this direction avoids, and at 1.085 this is not near it — and **the real reading is a playtest**. Report `sim-reports/benchmarks/2026-09-18T18-49-29-930Z-gymrun-randomizer-21-400.json` |
| `randomizer-21` · `d4e080`, 400 | `ai-7-tiers-reach-the-app`, pinned | RETUNE | 1.0% | **1.085** | **the same patch on the merged tree**, 2026-09-18, after `origin/main`'s wild-encounter swap fix came in. **It reproduces the row directly above to the digit** — 1.085 mean gyms, 1.0% completion, and all eight gym columns identical at 73.6 / 75.7 / 67.9 / 80.4 / 63.6 / 73.7 / 57.1 / 66.7 on parties of 250 / 144 / 84 / 46 / 33 / 19 / 14 / 6. **That reproduction is the point of the row rather than an afterthought.** The row above is stamped `ai-6` and this tree is `ai-7`, and this table's standing rule is to read down a prefix and never across a yardstick that moved. `AI_VERSION` moved here for a real reason — the app had been pinning `greedyAiPolicy` on its run options, so the shipped game played `GREEDY_BASELINE` behind a card reading Rookie — but that patch's own claim was that **`--ai pinned` is `GREEDY_BASELINE` and is untouched by it**, so a pinned row may be read across the bump. This row is that claim tested rather than taken: 400 seeds, every figure identical, on a tree where the constant's name is different. **Recorded, not chased.** Report `sim-reports/benchmarks/2026-09-18T19-38-51-813Z-gymrun-randomizer-21-400.json` |
| `randomizer-22` · `622777`, 400 | `ai-7-tiers-reach-the-app`, pinned | RETUNE | 1.0% | **1.055** | **the user-locked move cut**, 2026-09-19. Aura Wheel, Hyperspace Fury and Double Shock out of `data/movePools.ts`; read against the row directly above, which is the same population, the same AI and the same tree one commit earlier: **−0.03 mean gyms, completion unmoved at 1.0%.** Gym clear rates 73.6 / 75.7 / 58.8 / 80.5 / 61.3 / 82.4 / 69.2 / 66.7 on parties of 250 / 144 / 85 / 41 / 31 / 17 / 13 / 6, against 73.6 / 75.7 / 67.9 / 80.4 / 63.6 / 73.7 / 57.1 / 66.7 on 250 / 144 / 84 / 46 / 33 / 19 / 14 / 6. **Gyms 1 and 2 reproduce to the digit on identical party counts, and that is the row's real content**: all three cut moves are band 4 and 5, segments 0 to 2 draw bands 1 to 3, so the first two columns *cannot* move and do not. Every column that does move is downstream of the first draw the cut can reach. **Gym 3 is −9.1pt on 84 and 85 parties** and is the only moved column deep enough to argue about; from gym 4 on it is 41 parties falling to 6 and the figures swing both ways (+8.7pt at gym 6, +12.1pt at gym 7) on seventeen and thirteen runs. The mean moving three hundredths while the pool loses 3 of 397 entries is the size of effect the change argues for, and the direction is not interpretable at this sample. **Recorded, not chased**: nothing was tuned against this run, and the cut was aimed at a move slot that could not be spent rather than at this number. Report `sim-reports/benchmarks/2026-09-19T02-58-45-188Z-gymrun-randomizer-22-400.json` |
| `randomizer-23` · `431cfa`, 400 | `ai-7-tiers-reach-the-app`, pinned | RETUNE | 0.75% | **0.995** | **the inert-ability cut, and Double Shock re-admitted**, 2026-09-19. `data/abilities.ts` from 310 entries to 277; read against the row directly above: **−0.06 mean gyms, completion 1.0% to 0.75%**, which is four runs in four hundred becoming three. Gym clear rates 76.6 / 70.7 / 70.0 / 76.9 / 75.0 / 68.8 / 87.5 / 50.0 on parties of 239 / 133 / 70 / 39 / 28 / 16 / 8 / 6, against 73.6 / 75.7 / 58.8 / 80.5 / 61.3 / 82.4 / 69.2 / 66.7 on 250 / 144 / 85 / 41 / 31 / 17 / 13 / 6. **This row is a re-roll rather than an A/B, and that is the reading.** Every Pokemon in the game draws an ability, so unlike the move cut above — which could not touch a segment that draws bands 1 to 3, and visibly did not — this moves every seed from its first node: 239 parties reach gym 1 where 250 did, and the ones that get there clear it 3.0pt more often. The mechanism argues both directions at once, since every opponent now has an ability that does something and so does every party member, and at 400 seeds neither is separable from the other or from the re-roll. **Recorded, not chased**: nothing was tuned against this run, and the cut was aimed at an ability slot indistinguishable from having no ability rather than at this number. Report `sim-reports/benchmarks/2026-09-19T10-25-09-688Z-gymrun-randomizer-23-400.json` |
