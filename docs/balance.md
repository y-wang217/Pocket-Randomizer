...
| `randomizer-19` · `a036d6`, 400 | `ai-6-spent-item`, pinned | RETUNE | 0.0% | **0.56** | **the band recut and the level curve**, 2026-09-17. Read against the row directly above: **+0.015 mean gyms, completion unmoved at zero.** Gym clear rates: 56.4 / 47.4 / 45.5 / 53.8 / 40.0 / 50.0 / 100 / 0.0 against 48.3 / 49.2 / 25.5 / 62.5 / 50.0 / 0.0 — **gym 1 is +8.1pt on 250 parties**, which is the only column with enough behind it to carry a claim; gym 3 reads +20pt on 44 parties and gyms 4 onward are single figures either way. **The direction is opposite to the one the patch's own report predicted, and that is the finding.** The report measured the per-slot chance that a STAB super-effective hit one-shots rising 14.2% to 22.3%, because the damage formula's level term doubles between level 7 and level 15 while median HP grows 1.69x. It does rise. What it could not price is that closing `MOVESET.stabWindow` matters more: gym 1's measured band mix is **83/17/0** where the leak was producing **61/34/5**, so a player meets fewer band-2 moves and no band-3 moves at all, and wins more often despite each individual hit being likelier to kill. Two different measurements, and only one of them is the game. An interim reading of 20.4% was carried mid-patch and was wrong: it came off the `ladder` policy's first sample, which is `random` (22.7% here), not the pinned `greedy` this column is read against — **"read down a prefix" applies to the policy too.** This patch is four changes at once (five bands, the closed window, the stretched curve, the gym's two-page offer), so no single one of them owns this delta and the row does not claim otherwise. **Recorded, not chased**: nothing here was tuned against this run, and the author's standing ruling is that clear rate is not the target — an earlier build averaged 3 gyms and cleared all 8 consistently, which is the failure mode this direction is meant to avoid. Unmeasured as usual past the early segments: 13 parties reached gym 4 and 5 reached gym 5, so band 4 and band 5 — the whole point of the recut's top end — appear in almost no fight in this population. Report `sim-reports/benchmarks/2026-09-17T19-36-25-601Z-gymrun-randomizer-19-400.json` |
