...
| `randomizer-22` · `637670`, 400 | `ai-7-tiers-reach-the-app`, pinned | RETUNE | **0.0%** | **0.92** | **the region composition patch, checkpoint 1**, 2026-09-19. Read against the row directly above, the same population and the same tree one commit earlier: **−0.165 mean gyms, and the one completion is gone.** Three changes, all in how a route's shape is drawn and all pushing the same way: a per-route ceiling of two rests and two shops, the deletion of `restStepsPerGuarantee` (which had *guaranteed* a double rest on 100% of segment 5 and 6 routes), and a new floor of battle-only steps at `floor(steps / 2)`. The map measurements are the clearer reading and they are in `generation.md` section 52: routes offering three or more rests or shops go from up to 26% to **zero**, the five-shop route is gone, forced fights per route go 1.43 → 2.54 in the middle segments, and a greedy non-fight walk through a whole run drops from **33.6 of 46 steps to 25.2 of 45**. **Recorded, not chased.** Nothing was tuned against this run and the standing policy forbids retuning between checkpoints; `battleStepsPerGuarantee` is the dial if the number is to come back, and moving it from 2 to 3 returns the opening segments to roughly their old pressure while keeping the caps. **One consequence is louder than the mean**: no seed in 400 now reaches an evolution fork, because a fork needs a branching species in the party at a gym clear, so Stage 4.9's headline mechanic is close to unreachable at this difficulty. That is a finding for the author, not a number to tune away. Report `sim-reports/2026-09-19T09-58-50-455Z-gymrun-randomizer-22-400.json` |
