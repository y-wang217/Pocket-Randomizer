...
- Never a hedge word (risky, safe, strong, weak, good, bad, worth) on any surface. Amended 2026-09-19 (D3): **six live violations**, not four, measured on the tree at `e16decd` — `categoryInfo.ts:48`, `statusInfo.ts:125`, `statusInfo.ts:139`, `statusInfo.ts:171`, `statusInfo.ts:242`, and `bandInfo.ts:75` (the line the four-item list gave as `bandInfo.ts:68`, which is now a band label). They are milestone M0.3. The lint that holds this rule carries no allowlist: `statusInfo.ts:88`'s label is renamed to "Toxic", matching the TOX chip in section 2, rather than exempted.
